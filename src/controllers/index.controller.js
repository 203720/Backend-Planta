...
const getBomba=async(req,res)=>{
    const response=await pool.query('SELECT * FROM bomba')
    res.json(response.rows)
}

const insertBomba=async(req,res)=>{
    const date=  new Date().toISOString()
    const fecha=date.slice(0,10)
    const hora=date.slice(11,19)
    const response=await pool.query('INSERT INTO bomba (hora,fecha) VALUES ($1,$2)',[hora,fecha])
    res.json({
        data:response.rows,
